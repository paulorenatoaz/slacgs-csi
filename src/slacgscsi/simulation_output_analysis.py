...
def prepare_dataframe(data):
    ...
    ids = []
    for entry in data:
        sim_data = entry['simulation_data']
        ids.append(entry['id'])
        for n_samples, metrics in zip(sim_data['n_samples_list'], zip(sim_data['error_rate_list'], sim_data['simulation_report']['mean_precision_class_0'].values(),
                                            sim_data['simulation_report']['mean_fscore_class_0'].values(), sim_data['simulation_report']['mean_precision_class_1'].values(),
                                            sim_data['simulation_report']['mean_fscore_class_1'].values(), sim_data['simulation_report']['mean_training_time'].values())):
            df_list.append({
                'n_features': sim_data['n_features'],
                'n_samples': n_samples,
                ...
                'duration(h)': sim_data['duration(h)'],
                'test_size': sim_data['test_size'],
                'max_gmm_components': sim_data['max_gmm_components'],
                'pca_retained_variance': sim_data['pca_retained_variance'],
            })
...
def plot_metrics_vs_n(df, ids):
    ...
    for metric in metrics:
        ...
        crossvalidation = True if df['test_size'].unique() == [0] else False
        ...
        analysis_method = 'cross-validation=' if crossvalidation else 'train_test_split=' + str(
            test_size)
        plt.title(f'Simulations {ids}\n{metric.replace("_", " ").title()} vs Number of Samples\n ' +
                  'pca = ' + str(pca_retained_variance) + '; max_gmm_comp = ' + str(max_gmm_components) +
                  '; method: ' + analysis_method)
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(PROJECT_DIR, 'data', 'outputs', 'graphs',  f'simulations_{ids}_{metric}_vs_n.png'))
        plt.show()
...
def plot_metrics_vs_dimensions(df, ids):
    ...
    for metric in metrics:
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(PROJECT_DIR, 'data', 'outputs', 'graphs',  f'simulations_{ids}_{metric}_vs_dim.png'))
        plt.show()
...
def plot_duration_vs_dimensions(df, ids):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(PROJECT_DIR, 'data', 'outputs', 'graphs',  f'simulations_{ids}_duration_vs_dim.png'))
    plt.show()
...
def plot_training_time_vs_n(df, ids):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(PROJECT_DIR, 'data', 'outputs', 'graphs',  f'simulations_{ids}_training_time_vs_n.png'))
    plt.show()
...
def plot_training_time_vs_dimensions(df, ids):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(PROJECT_DIR, 'data', 'outputs', 'graphs',  f'simulations_{ids}_training_time_vs_dim.png'))
    plt.show()

def generate_simulations_plots():
    json_file_path = os.path.join(PROJECT_DIR, 'data', 'outputs', 'simulation_results.json')
